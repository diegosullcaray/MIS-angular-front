...
## ⏳ FASE 7 — Backend Real: Spring Boot + PostgreSQL (PENDIENTE — especificación lista)

> Especificación completa en [Backend Schema](../02-arquitectura/01-backend-schema.md) y DDL en [Database Schema](../02-arquitectura/02-database-schema.sql).

- [ ] **F7-01** · Scaffold `mis-backend` (Java 21, Spring Boot 3.3, módulos `shared/auth/accesos/sistemas` — reglas BE-01..BE-07)
- [ ] **F7-02** · Flyway `V1__baseline.sql` = doc 07 (esquemas iam/sistemas/auth/auditoria, RLS, roles de BD)
- [ ] **F7-03** · Módulo `auth`: login + OTP (TTL 3 min, 5 intentos, lockout) + JWT con `jti` en `auth.sesiones`
- [ ] **F7-04** · Módulos `accesos` y `sistemas`: endpoints del contrato §4 del doc 04
- [ ] **F7-05** · Auditoría: `SET LOCAL app.usuario_id/app.trace_id` por transacción + bitácora `auditoria.accesos`
...
> Guía completa en [Guía Sistemas Hijos](../02-arquitectura/03-guia-sistemas-hijos.md) (ejemplo: `mis-remote-reportes`, puertos 4205/8085).

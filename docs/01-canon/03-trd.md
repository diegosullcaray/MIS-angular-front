...
│
├── public/
│   └── federation.manifest.json  ← Solo passthrough de raíz (favicon, manifest). NUNCA imágenes aquí.
│
...
> ⚠️ **Convención de assets:** las imágenes van en `src/assets/`, no en `public/`. `angular.json`
> registra dos entradas de `assets` en el target `esbuild`: `public` (passthrough de raíz —

...
> Las reglas de este documento son la aplicación concreta del Pilar 2 (usabilidad macOS)
> de [`00-vision-producto.md`](./00-vision-producto.md): un solo breadcrumb, cards a
> ancho completo, `SelectButton` en vez de wizards, mensajería efímera. Léase ese
> documento primero si es la primera vez que se toca el Host.
...
├── LoginComponent (Standalone) ← Credenciales + LoadSpinnerComponent (transición de 5s antes de navegar); paso OTP MFA deshabilitado en la UI por ahora

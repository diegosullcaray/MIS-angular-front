...
@Component({
  selector: 'app-report-cra-v1p2',
  templateUrl: './report-cra-v1p2.component.html',
  styleUrls: ['./report-cra-v1p2.component.scss']
})
export class ReportCraV1p2Component implements OnInit, OnDestroy {
  report: ReportT;
  activeHier: boolean;
  confHier1: any;
  ...
  private combineSelections() {
    combineLatest([this.filter$, this.level$]).subscribe(([filter, level]) => {
      ...
      this.renderTable_1(params, { find: '_04', index: 3 })

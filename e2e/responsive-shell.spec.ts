 ...
 * reservado para escritorio para que las acciones móviles entren completas;
...
test.describe('Shell responsive — mobile (< 640px, breakpoint `sm` de Tailwind)', () => {
  ...
  test('el breadcrumb del header se oculta para dar espacio a las acciones', async ({ page }) => {
    const shell = new ShellPage(page);
    await shell.ir();
...
test.describe('Shell responsive — desktop (>= 640px)', () => {
  ...
  test('en claro usa el wallpaper de escritorio (wallpaper.png), no el de mobile', async ({ page }) => {
    const shell = new ShellPage(page);
    await shell.fijarTema('claro');
    await shell.ir();

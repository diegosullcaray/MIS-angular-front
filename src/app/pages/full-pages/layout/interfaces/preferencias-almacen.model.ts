...
/**
 * Dónde se guardan las preferencias. `PreferenciasService` solo conoce esta
 ...
 * una contra el backend sin tocar el servicio ni el modelo.
 *
...
/**
 * Implementación nula: recuerda dentro de la propia instancia y nada más. Es el
 * valor por defecto del token para que la aplicación nunca dependa de que
 * exista un almacenamiento — en un test, o en un render sin `window`, las
 * preferencias funcionan igual, solo no sobreviven.
 */
...
export const REPOSITORIO_PREFERENCIAS = new InjectionToken<RepositorioPreferencias>('REPOSITORIO_PREFERENCIAS', {
  providedIn: 'root',
  // El almacén real (`localStorage`) lo enchufa `app.config.ts`; acá solo se
  // garantiza que inyectarlo nunca falle por falta de proveedor.

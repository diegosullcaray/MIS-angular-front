 ...
 * Devuelve `null` si la fila no trae un nodo completo, como la fila de totales.
 */
export function nodoDeFila(fila: Record<string, unknown>): HierarquiaNodo | null {
  ...
  return {
    ...
    des_rel: String(fila['descripcion'] ?? fila['rdesjer'] ?? fila['DESCRIPCION'] ?? ''),
  };
}

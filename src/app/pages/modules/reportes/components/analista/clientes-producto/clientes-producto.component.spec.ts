...
import { ClientesProductoComponent } from './clientes-producto.component';
import { ClientesProductoService } from '../../../services/analista/clientes-producto.service';
...
describe('ClientesProductoComponent', () => {
  let servicioFalso: { obtenerAsesores: ReturnType<typeof vi.fn>; obtenerClientesProducto: ReturnType<typeof vi.fn> };

  beforeEach(() => {
    servicioFalso = {
      obtenerAsesores: vi.fn().mockReturnValue(of([ASESOR])),
      obtenerClientesProducto: vi
        .fn()
        .mockReturnValue(of({ tabla1: tabla({ body: [{ nom_cli: 'a' }] }), tabla2: tabla(), tabla3: tabla() })),
    };
    TestBed.configureTestingModule({
      imports: [ClientesProductoComponent],
      providers: [
        { provide: ClientesProductoService, useValue: servicioFalso },
        ToastService,
        MessageService,
  ...
  function crear() {
    const fixture = TestBed.createComponent(ClientesProductoComponent);
  ...
  it('onAsesorSeleccionado() carga las 3 tablas del asesor elegido', () => {
    ...
    expect(servicioFalso.obtenerClientesProducto).toHaveBeenCalledWith({ tip_cod: 2, cod_rel: ASESOR.dni });
    expect(fixture.componentInstance['tabla1']().body).toEqual([{ nom_cli: 'a' }]);
  });

  it('avisa con un toast si falla la carga', () => {
    servicioFalso.obtenerClientesProducto.mockReturnValue(throwError(() => new Error('falló')));
    const fixture = crear();
  ...
  it('muestra una advertencia si las 3 tablas vienen vacías', () => {
    servicioFalso.obtenerClientesProducto.mockReturnValue(of({ tabla1: tabla(), tabla2: tabla(), tabla3: tabla() }));
    const fixture = crear();

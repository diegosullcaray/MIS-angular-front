...
describe('SegurosComponent', () => {
  let servicioFalso: { obtenerAsesores: ReturnType<typeof vi.fn>; obtenerSeguros: ReturnType<typeof vi.fn> };

  beforeEach(() => {
    servicioFalso = {
      obtenerAsesores: vi.fn().mockReturnValue(of([ASESOR])),
      obtenerSeguros: vi.fn().mockReturnValue(of({ tabla1: tabla({ body: [{ nom_cli: 'a' }] }) })),
    };
    TestBed.configureTestingModule({
      imports: [SegurosComponent],
      providers: [{ provide: AsesorSecService, useValue: { obtenerAsesores: servicioFalso.obtenerAsesores } }, { provide: SegurosService, useValue: servicioFalso }, ToastService, PrimeNgMessageService],
    });
  });

  function crear() {
    const fixture = TestBed.createComponent(SegurosComponent);
  ...
  it('onAsesorSeleccionado() carga la tabla del asesor elegido', () => {
    ...
    expect(servicioFalso.obtenerSeguros).toHaveBeenCalledWith({ tip_cod: 2, cod_rel: ASESOR.dni });
    expect(fixture.componentInstance['tabla1']().body).toEqual([{ nom_cli: 'a' }]);
  });

  it('avisa con un toast si falla la carga', () => {
    servicioFalso.obtenerSeguros.mockReturnValue(throwError(() => new Error('falló')));
    const fixture = crear();

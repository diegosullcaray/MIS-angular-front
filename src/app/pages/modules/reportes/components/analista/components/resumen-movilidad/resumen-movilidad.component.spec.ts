...
describe('ResumenMovilidadComponent', () => {
  let servicioFalso: { obtenerAsesores: ReturnType<typeof vi.fn>; obtenerResumenMovilidad: ReturnType<typeof vi.fn> };

  beforeEach(() => {
    servicioFalso = {
      obtenerAsesores: vi.fn().mockReturnValue(of([ASESOR])),
      obtenerResumenMovilidad: vi.fn().mockReturnValue(of({ tabla1: tabla({ body: [{ nom_cli: 'a' }] }) })),
    };
    TestBed.configureTestingModule({
      imports: [ResumenMovilidadComponent],
      providers: [
        { provide: ResumenMovilidadService, useValue: servicioFalso },
        ToastService,
        MessageService,
  ...
  function crear() {
    const fixture = TestBed.createComponent(ResumenMovilidadComponent);
  ...
  it('onAsesorSeleccionado() carga la tabla del asesor elegido', () => {
    ...
    expect(servicioFalso.obtenerResumenMovilidad).toHaveBeenCalledWith({ tip_cod: 2, cod_rel: ASESOR.dni });
    expect(fixture.componentInstance['tabla1']().body).toEqual([{ nom_cli: 'a' }]);
  });

  it('avisa con un toast si falla la carga', () => {
    servicioFalso.obtenerResumenMovilidad.mockReturnValue(throwError(() => new Error('falló')));
    const fixture = crear();
  ...
  it('muestra una advertencia si la tabla viene vacía', () => {
    servicioFalso.obtenerResumenMovilidad.mockReturnValue(of({ tabla1: tabla() }));
    const fixture = crear();

...
import { ColocacionesDiariaService } from '../../../services/analista/colocaciones-diaria.service';
...
describe('ColocacionesDiariaComponent', () => {
  let servicioFalso: { obtenerAsesores: ReturnType<typeof vi.fn>; obtenerColocacionesDiaria: ReturnType<typeof vi.fn> };

  beforeEach(() => {
    servicioFalso = {
      obtenerAsesores: vi.fn().mockReturnValue(of([ASESOR])),
      obtenerColocacionesDiaria: vi
        .fn()
        .mockReturnValue(of({ tabla1: tabla({ body: [{ nom_cli: 'a' }] }), tabla2: tabla(), tabla3: tabla() })),
    };
    TestBed.configureTestingModule({
      imports: [ColocacionesDiariaComponent],
      providers: [
  ...
  function crear() {
    const fixture = TestBed.createComponent(ColocacionesDiariaComponent);
  ...
  it('onAsesorSeleccionado() carga las 3 tablas del asesor elegido', () => {
    ...
    expect(servicioFalso.obtenerColocacionesDiaria).toHaveBeenCalledWith({ tip_cod: 2, cod_rel: ASESOR.dni });
    expect(fixture.componentInstance['tabla1']().body).toEqual([{ nom_cli: 'a' }]);
  });

  it('avisa con un toast si falla la carga', () => {
    servicioFalso.obtenerColocacionesDiaria.mockReturnValue(throwError(() => new Error('falló')));
    const fixture = crear();

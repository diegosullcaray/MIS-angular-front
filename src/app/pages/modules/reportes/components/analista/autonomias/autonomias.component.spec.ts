...
describe('AutonomiasComponent', () => {
  let servicioFalso: { obtenerAsesores: ReturnType<typeof vi.fn>; obtenerAutonomias: ReturnType<typeof vi.fn> };

  beforeEach(() => {
    servicioFalso = {
      obtenerAsesores: vi.fn().mockReturnValue(of([ASESOR])),
      obtenerAutonomias: vi.fn().mockReturnValue(of({ tabla1: tabla({ body: [{ nom_cli: 'a' }] }) })),
    };
    TestBed.configureTestingModule({
      imports: [AutonomiasComponent],
      providers: [{ provide: AutonomiasService, useValue: servicioFalso }, ToastService, MessageService, PrimeNgMessageService],
    });
  });

  function crear() {
    const fixture = TestBed.createComponent(AutonomiasComponent);
  ...
  it('onAsesorSeleccionado() carga la tabla del asesor elegido', () => {
    ...
    expect(servicioFalso.obtenerAutonomias).toHaveBeenCalledWith({ tip_cod: 2, cod_rel: ASESOR.dni });
    expect(fixture.componentInstance['tabla1']().body).toEqual([{ nom_cli: 'a' }]);
  });

  it('avisa con un toast si falla la carga', () => {
    servicioFalso.obtenerAutonomias.mockReturnValue(throwError(() => new Error('falló')));
    const fixture = crear();
  ...
  it('muestra una advertencia si la tabla viene vacía', () => {
    servicioFalso.obtenerAutonomias.mockReturnValue(of({ tabla1: tabla() }));
    const fixture = crear();

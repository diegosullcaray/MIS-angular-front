...
describe('CeroCuotasComponent', () => {
  let servicioFalso: { obtenerAsesores: ReturnType<typeof vi.fn>; obtenerCeroCuotas: ReturnType<typeof vi.fn> };

  beforeEach(() => {
    servicioFalso = {
      obtenerAsesores: vi.fn().mockReturnValue(of([ASESOR])),
      obtenerCeroCuotas: vi.fn().mockReturnValue(of({ tabla1: tabla({ body: [{ nom_cli: 'a' }] }) })),
    };
    TestBed.configureTestingModule({
      imports: [CeroCuotasComponent],
      providers: [{ provide: CeroCuotasService, useValue: servicioFalso }, ToastService, MessageService, PrimeNgMessageService],
    });
  });

  function crear() {
    const fixture = TestBed.createComponent(CeroCuotasComponent);
  ...
  it('onAsesorSeleccionado() carga la tabla del asesor elegido', () => {
    ...
    expect(servicioFalso.obtenerCeroCuotas).toHaveBeenCalledWith({ tip_cod: 2, cod_rel: ASESOR.dni });
    expect(fixture.componentInstance['tabla1']().body).toEqual([{ nom_cli: 'a' }]);
  });

  it('avisa con un toast si falla la carga', () => {
    servicioFalso.obtenerCeroCuotas.mockReturnValue(throwError(() => new Error('falló')));
    const fixture = crear();
  ...
  it('muestra una advertencia si la tabla viene vacía', () => {
    servicioFalso.obtenerCeroCuotas.mockReturnValue(of({ tabla1: tabla() }));
    const fixture = crear();

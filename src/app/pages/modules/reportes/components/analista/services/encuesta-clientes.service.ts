...
export class EncuestaClientesService {
  ...
  obtenerClientes(asesor: { tip_cod: number; cod_rel: string }): Observable<TablaReporteResultado> {
    return this.reportes.getRegularData(COD_ANALISTA.encuestaClientes, asesor).pipe(map(mapearBloqueReporte));
  }
  ...
  obtenerCiiu(): Observable<CiiuOpcion[]> {
    return this.reportes.getRegularData(COD_ANALISTA.encuestaClientesCiiu, {}).pipe(map((r) => mapearBloqueReporte(r).body as unknown as CiiuOpcion[]));
  }
  ...
  guardarEncuesta(cliente: FilaReporte, respuestas: EncuestaClienteForm): Observable<unknown> {
    return this.secciones.postRegularUpdate(COD_ANALISTA.encuestaClientesGuardar, { ...cliente, reaccion: formularioAJson(respuestas) });
  }
}

...
/** "Gestión de Cartera Reasignada" (`leg/com/rma/adm/gest_cart_her-flujo`). */
@Component({
...
export class GestionCarteraReasignadaComponent extends ReporteBloquesBase {
  ...
  protected override readonly titulos = [
    'GESTIÓN DE CARTERA REASIGNADA BASE FLUJO',
    'GESTIÓN DE CARTERA REASIGNADA BASE FLUJO (DETALLE)',
  ] as const;

  protected override consultar(nodo: NodoConsulta): Observable<TablaReporteResultado[]> {
    return this.servicio.gestionCarteraReasignadaFlujo(nodo, this.mostrarPor(), this.fechaBase());
  }
}

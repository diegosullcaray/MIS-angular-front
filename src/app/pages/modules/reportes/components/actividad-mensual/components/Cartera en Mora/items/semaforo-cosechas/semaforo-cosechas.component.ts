...
/** "Semáforo de Cosechas" (`leg/com/rma/adm/sema-cosechas`). */
@Component({
  selector: 'app-mensual-semaforo-cosechas',
  standalone: true,
  imports: [ReporteSimpleComponent, SelectFiltroComponent],
  templateUrl: './semaforo-cosechas.component.html',
  styleUrl: './semaforo-cosechas.component.css',
})
export class SemaforoCosechasComponent extends ReporteSimpleBase {
  ...
  protected override consultar(nodo: NodoConsulta): Observable<ReporteBloqueUnico> {
    return this.servicio.semaforoCosechas(nodo, this.producto(), this.subproducto(), this.maduracion(), this.tipo());
  }
}
